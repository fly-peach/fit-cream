...
设置 ACL 为私有，返回长期有效的签名 URL（默认约 100 年）供访问 / 传给 DashScope 多模态接口。
...
def _get_bucket() -> Bucket:
    if not is_oss_configured():
        raise RuntimeError(
            "OSS 配置不完整，请检查环境变量 OSS_ACCESS_KEY_ID / "
...
def _object_key(user_id: UUID | str, content_type: str) -> str:
...
def upload_chat_image(
    ...
    """上传聊天图片到 OSS 私有路径，返回长期有效签名的完整 URL。

    聊天图片涉及用户隐私，ACL 设为私有；返回的签名 URL 默认约 100 年有效
    （由 settings.OSS_SIGN_URL_EXPIRES 控制），可直接嵌入前端或传给 DashScope。
    """
    bucket = _get_bucket()
