...
async def get_exercise_group_tool(
    ...
    使用场景：
    - 为用户选定的身材目标生成训练计划时：以返回的动作组（胸/背/肩/腿/核心…）为候选底座编排每日计划
    - 推荐动作组是**基准配置而非硬性约束**：每肌群动作数量可按用户水平、器械条件与
      单次时长增删替换；每个力量训练日安排 4~6 个动作（不含热身），其中必须以
      「拉伸」组 1-2 个动作收尾（练后拉伸为硬性要求，有氧日可豁免）
    - 替换/补充同肌群动作时仍须用 get_exercises_tool 检索获取动作 ID，禁止编造；
      伤病或器械缺失时优先在同组内换更安全的器械版（史密斯/固定器械 > 自由重量）
    ...
    try:
        async with session_scope() as db:
            ...
            return {
                ...
                "note": (
                    "推荐动作组为该身材的安全入门基准，非硬性约束：每肌群动作数量可按"
                    "用户水平/器械/时长增删替换（替换动作须经 get_exercises_tool 检索），"
                    "每个力量训练日 4-6 个动作，末组「拉伸」1-2 个收尾为硬性要求。"
                ),
            }
    except Exception as e:
        return error_response(e)
