...
class MemoryUpdateMiddleware(AgentMiddleware):
    ...
    def after_agent(self, state: AgentState, runtime: Runtime) -> dict[str, Any] | None:
        ...
        logger.info(
            ...
            f"accumulated_tokens={self._accumulated_tokens}/{self.trigger_tokens} | "
            f"messages_since_update={self._messages_since_last_update}"
        )
    ...
    async def _extract_memories(self, messages: list) -> None:
        ...
        """
        try:
            from src.agents.harness.memory.pipeline import get_memory_pipeline
