...
- MemoryConsolidationLog: 记忆整合日志

使用 pgvector 扩展存储向量，支持语义检索。
模型继承独立 MemoryBase（非 app Base），不进 app Base.metadata，
建表走 MemoryStore.init_db()（MemoryBase.metadata.create_all）。

用法：
    from src.agents.models.memory import EpisodicMemory, SemanticMemory, ProceduralMemory
"""
