...
class DietMeal(Base):
    ...
    __table_args__ = (
        Index("idx_diet_meal_user_date", "user_id", "meal_date"),
    )
    ...
    custom_food_item_id: Mapped[Optional[UUID]] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("custom_food_items.id", ondelete="SET NULL"), nullable=True, index=True)
...
class CustomFoodItem(Base):
    ...
    diet_meals: Mapped[List["DietMeal"]] = relationship(back_populates="custom_food_item")
