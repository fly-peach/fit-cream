...
def test_model_sees_redacted_queue_args_but_state_kept():
    ...
    # 模型实际看到的队列工具入参被裁剪（todos 为占位串）
    seen = model.seen_messages
    queue_ai = next(m for m in seen if getattr(m, "tool_calls", None))
    seen_args = queue_ai.tool_calls[0]["args"]
    assert isinstance(seen_args["todos"], str)
    assert "省略" in seen_args["todos"]
