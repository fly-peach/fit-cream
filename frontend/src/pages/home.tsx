...
/** 微信品牌图标（lucide-react 已移除，内联 SVG 替代） */
function WechatIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className}>
      <path d="M8.691 2.188C3.891 2.188 0 5.476 0 9.53c0 2.212 1.17 4.203 3.002 5.55a.59.59 0 0 1 .213.665l-.39 1.48c-.019.07-.048.141-.048.213 0 .163.13.295.29.295a.326.326 0 0 0 .167-.054l1.903-1.114a.864.864 0 0 1 .717-.098 10.16 10.16 0 0 0 2.837.403c.276 0 .543-.027.811-.05-.857-2.578.157-4.972 1.932-6.442 1.703-1.415 3.882-1.98 5.853-1.838-.576-3.583-4.196-6.348-8.596-6.348zM5.785 5.991c.642 0 1.162.529 1.162 1.18a1.17 1.17 0 0 1-1.162 1.178A1.17 1.17 0 0 1 4.623 7.17c0-.651.52-1.18 1.162-1.18zm5.813 0c.642 0 1.162.529 1.162 1.18a1.17 1.17 0 0 1-1.162 1.178 1.17 1.17 0 0 1-1.162-1.178c0-.651.52-1.18 1.162-1.18zm5.34 2.867c-1.797-.052-3.726.603-5.152 1.828-1.72 1.478-2.595 3.694-1.78 6.01.815 2.316 3.266 3.836 5.95 3.836.553 0 1.08-.076 1.59-.212a.72.72 0 0 1 .59.082l1.56.914a.27.27 0 0 0 .138.045c.133 0 .24-.108.24-.243 0-.06-.023-.118-.038-.174l-.326-1.216a.48.48 0 0 1 .176-.546C21.063 17.867 22 16.283 22 14.531c0-3.142-2.54-5.621-5.062-5.673zm-2.036 2.87c.535 0 .969.44.969.982a.976.976 0 0 1-.969.983.976.976 0 0 1-.969-.983c0-.542.434-.982.97-.982zm4.072 0c.535 0 .969.44.969.982a.976.976 0 0 1-.969.983.976.976 0 0 1-.969-.983c0-.542.434-.982.969-.982z" />
    </svg>
  );
}
...
function Footer() {
  ...
  const contactWechat = import.meta.env.VITE_CONTACT_WECHAT as string | undefined;
  const showContact = Boolean(contactPhone || contactWechat);
  return (
    <footer className="border-t border-white/10 bg-emerald-950">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          ...
          {showContact && (
            <div>
              <p className="text-sm font-semibold text-emerald-100">联系相关人员</p>
              <ul className="mt-4 space-y-2.5">
                {contactPhone && (
                  <li>
                    <a
                      href={`tel:${contactPhone}`}
                      className="inline-flex items-center gap-2 text-sm text-emerald-200/60 transition-colors hover:text-emerald-300"
                    >
                      <PhoneIcon className="size-3.5 text-emerald-400/70" />
                ...
                {contactWechat && (
                  <li>
                    <span className="inline-flex items-center gap-2 text-sm text-emerald-200/60">
                      <WechatIcon className="size-3.5 text-emerald-400/70" />
                      <span>微信：{contactWechat}</span>
                    </span>
                  </li>
                )}
              </ul>
