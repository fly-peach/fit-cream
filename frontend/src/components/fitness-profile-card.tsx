/**
 * 个人资料卡（个人中心）：基本资料 + 健身画像整合为单卡片多 tab。
 *
 * 健身画像自取数据 GET /users/me/fitness-profile，按 5 个 tab 分组展示 Intake 五维，
 * 全字段可编辑（与 FormCard 不同：档案已存值也可修改），每 tab 独立保存
 * PUT /users/me/fitness-profile（仅提交该维度有值字段）；
 * 基本资料 tab 由父组件传入（昵称/生日/身高/体重/性别/目标，走 /users/me 接口）。
 *
...
const PROFILE_META_KEYS = new Set(["id", "user_id", "created_at", "updated_at"])

function optionLabel(field: FormFieldDef, value: string): string {
  return field.options?.find((o) => o.value === value)?.label ?? value
}

export function FitnessProfileCard({ basicTab }: { basicTab?: ReactNode }) {
  ...
  const handleSave = async (tabId: string) => {
    ...
    setError("")
    try {
      const updated = await api.put<FitnessProfile>(
        "/users/me/fitness-profile",
        payload
      )
      setProfile(updated)
      setSavedTab(tabId)
    ...
    () =>
      profile
        ? Object.entries(profile).filter(
            ([k, v]) => !PROFILE_META_KEYS.has(k) && v !== null && v !== ""
          ).length
        : 0,
    [profile]
  )
      ...
      <CardHeader>
        ...
        {profile && filledCount > 0 && (
          <p className="text-xs text-emerald-600/60">
            健身画像已填写 {filledCount} 项，聊天与计划设计共用这份档案
          </p>
        )}
      </CardHeader>
