...
CheckinOut：id, user_id, plan_day_id, date, duration_min, mood, note, created_at, exercises[]
...
| exercises | list[CheckinExerciseCreate] | 动作记录列表（含 exercise_id, sets_done, reps_done, weight_kg） |
...
**响应：`ResponseModel[list[AchievementOut]]`**

AchievementOut：id, type, name, description, icon, unlocked_at（按解锁时间降序）
...
|------|-----|
| 方法 | GET |
| 路径 | `/api/achievements/all` |

**响应：`ResponseModel[list[AchievementProgressOut]]`**

AchievementProgressOut：type, name, description, icon, unlocked (bool), unlocked_at (Optional)
...
| streak_100 | 百日坚持 | 连续打卡 100 天 |
| first_plan | 初次规划 | 首次创建训练计划 |
| total_50_workouts | 半百训练 | 累计完成 50 次训练 |
| total_100_workouts | 百炼成钢 | 累计完成 100 次训练 |
